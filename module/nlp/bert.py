...
from mytorch.data.wikitext2v2 import WikiText2, WikiText2Label, WikiText2Sample
...
class BERTEmbedding(nn.Module):
    def __init__(self, vocab_size: int, hidden_size: int, max_len: int):
        ...
        self.segment_embedding = nn.Embedding(
            num_embeddings=2, embedding_dim=hidden_size
        )
    ...
    def forward(self, sentences: Tensor, segments: Tensor) -> Tensor:
        ...
        assert token_embedding.shape == (batch_size, seq_size, self.hidden_size)
...
class BERTEncoder(nn.Module):
    def __init__(
        self, num_head: int, hidden_size: int, ffn_hidden_size: int, num_blocks: int
    ):
...
class MaskedLanguageModel(nn.Module):
    ...
    def forward(self, encoder_output: Tensor, masked_indices: Tensor) -> Tensor:
        ...
        index0 = torch.arange(batch_size).repeat_interleave(masked_token_size)
...
class BERTLossImpl(nn.modules.loss._Loss):
    ...
    def __call__(
        self, bert_output: BERTOutput, labels: WikiText2Label
    ) -> tuple[Tensor, Tensor]:
...
class BERTEvaluator(Evaluator):
    ...
    def mlm_err_batch(
        ...
        # top1 error rate
        top1_err = utils.topk_err(
            k=1,
            logits=bert_output.mlm,
            labels=labels.mlm.flatten(),
            mask=bert_output.mlm_mask.flatten(),
        )
        top5_err = utils.topk_err(
            k=5,
            logits=bert_output.mlm,
            labels=labels.mlm.flatten(),
            mask=bert_output.mlm_mask.flatten(),
...
class BERT(nn.Module):
    def __init__(
        ...
        if use_pytorch_encoder:
            ...
            self.encoder = BERTEncoder(
                num_head=num_head,
                hidden_size=hidden_size,  # type: ignore
                ffn_hidden_size=ffn_hidden_size,
                num_blocks=num_blocks,
            )
    ...
    def forward(self, sample: WikiText2Sample) -> BERTOutput:
        ...
        # return self.net(x)
        # 1. embedding
        embedding = self.embedding(sentences=sample.sentences, segments=sample.segments)
        # 2. encoder
