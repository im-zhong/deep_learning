...
from mytorch.net.cnn import MyAvgPool2d, MyConv2d, MyConv2dWithGroups, MyMaxPool2d


def test_MyConv2d():
    conv = MyConv2d(in_channels=1, out_channels=1, kernel_size=3, padding=1, stride=2)
    input = torch.rand(size=(2, 1, 8, 8))
    output = conv(input=input)
    assert output.shape == (2, 1, 4, 4)

    conv = MyConv2d(
        in_channels=1, out_channels=1, kernel_size=(3, 5), padding=(0, 1), stride=(3, 4)
    )
    input = torch.rand(size=(1, 1, 8, 8))
    output = conv(input=input)
    assert output.shape == (1, 1, 2, 2)

    conv = MyConv2d(in_channels=2, out_channels=3, kernel_size=2, padding=0, stride=1)
    input = torch.stack(
        [
            torch.arange(start=0, end=9, step=1).reshape(3, 3),
...
def test_my_conv2d_with_groups():
    ...
    torch_conv = nn.LazyConv2d(
        out_channels=out_channels, kernel_size=3, padding=1, groups=groups
    )
